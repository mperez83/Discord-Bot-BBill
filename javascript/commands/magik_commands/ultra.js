...
module.exports.run = async (bot, message, args) => {
    ...
    if (args.length > 0) {
        message.channel.send(`no parameters here, ${utilitiesModule.getRandomNameInsult(message.author)}`);
        return;
    }
        ...
        if (!foundURL) {
            ...
            rp(options)
                .then(function (response) {
                    ...
                    if (fileSize > 0.5) {
                        message.channel.send(`I can't make this go ultra, ${utilitiesModule.getRandomNameInsult(message.author)} (larger than 0.5mb)`);
                        return;
                    }
                    else {
                        gm(request(foundURL))
